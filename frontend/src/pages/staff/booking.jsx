...
import { fetchBookings, updateBookingStatus, reportIncident } from '@/services/mock/mockApi';
            ...
            {isLoading ? (
              ...
              <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-10">
                {filteredBookings.map((booking) => {
                  const theme = getStatusTheme(booking.status, booking.service_type);
                  return (
                    <div 
                        ...
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            ...
                            <div className="flex items-center gap-1.5 text-sm font-semibold text-gray-700 mt-0.5">
                              <Clock size={14} className="text-gray-400" /> {booking.booking_time}
                            </div>
                          </div>
                          <div>
                        ...
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex flex-col gap-0.5">
                              <p className="text-xs text-gray-500 truncate">{booking.pet_breed} • {booking.pet_weight}kg</p>
                              <div className="flex items-center gap-1 text-xs text-gray-600 truncate">
                                <Phone size={10} /> {booking.customer_name}
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="bg-gray-50 rounded-xl p-3 mb-5 mt-auto border border-gray-100 flex flex-col gap-2">
                          <div className="flex items-start gap-2">
                            <div className="w-5 h-5 rounded flex items-center justify-center bg-white border border-gray-200 shrink-0 mt-0.5">
                              {booking.service_type === 'Hotel' ? '🏨' : booking.service_type === 'Grooming' ? '✂️' : '🏥'}
                            </div>
                            <div>
                              <div className="text-[10px] font-bold text-gray-400 uppercase">Dịch vụ</div>
                              <div className="text-sm font-semibold text-gray-900 leading-tight">{booking.service_type}</div>
                            </div>
                          </div>
                          ...
                          {booking.room_or_slot_id && (
                            <div className="flex items-start gap-2 pt-2 border-t border-gray-200">
                              ...
                              <div>
                                <div className="text-[10px] font-bold text-gray-400 uppercase">Vị trí</div>
                                <div className="text-sm font-semibold text-chloro">{booking.room_or_slot_id}</div>
                              </div>
                            </div>
                          )}
                        </div>
