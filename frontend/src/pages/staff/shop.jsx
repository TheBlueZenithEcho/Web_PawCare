...
export default function ShopPOS() {
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      setCategories(['Tất cả', ...cats.map(c => c.category_name)]);
  ...
  const addToCart = (product) => {
    setCart(prev => {
      ...
      // Tìm variant_id thực từ full_variants, mặc định lấy variant đầu tiên
      const variant_id = product.full_variants && product.full_variants.length > 0 ? product.full_variants[0].variant_id : null;
      return [...prev, { product, quantity: 1, variant: product.variants[0] || '', variant_id }];
    });
  };
  ...
  const handleCheckout = async () => {
    ...
    const orderItems = cart.map(item => ({
      ...
      variant_id: item.variant_id || item.product.id, // Fallback
              ...
              <div className="w-full lg:w-[400px] xl:w-[450px] bg-white border-l border-gray-200 flex flex-col h-[calc(100vh-130px)] shrink-0 shadow-[-10px_0_15px_-3px_rgba(0,0,0,0.02)] z-20 transition-transform">
                ...
                <div className="p-4 md:p-6 border-t border-gray-200 bg-white">
                  ...
                  <button
                    ...
                    className={`w-full py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-md ${cart.length > 0
