    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-5xl h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className={`${viewMode === 'history' ? 'w-full flex-row' : 'w-1/2 flex-col'} flex border-r border-gray-100 bg-gray-50/50 transition-all duration-300`}>
            {/* Pet Info */}
            <div className={`p-6 shrink-0 ${viewMode === 'history' ? 'w-1/3 border-r border-gray-100' : 'w-full'}`}>
              <div className="bg-[#f8faff] border border-blue-100 rounded-xl p-5 shadow-sm">
                <div className="flex gap-4 items-center mb-5">
                  <div className="w-14 h-14 bg-blue-100 rounded-full flex items-center justify-center text-3xl shadow-sm border border-white">
                    {booking.pet_type === 'Mèo' ? '🐈' : '🐕'}
                  </div>
                  <div>
                    <h3 className="text-xl font-bold text-[#1a3b5c]">{booking.pet_name}</h3>
                    <p className="text-[#3b82f6] text-sm font-medium mt-1">{booking.pet_type} · {booking.pet_breed} · Lớn</p>
                  </div>
                </div>
                ...
                {warningText && (
                  <div className="bg-orange-50 border border-orange-200 text-orange-800 p-3 rounded-lg text-sm flex items-start gap-2 mb-4">
                ...
                <div className="bg-white border border-gray-100 rounded-lg overflow-hidden">
                  <div className="bg-gray-50 px-3 py-2 border-b border-gray-100 font-bold text-sm text-understory">
                    🏥 Hồ sơ sức khỏe
                  </div>
                  <div className="p-3 space-y-2 text-sm">
                    <div className="grid grid-cols-2 gap-2">
                      <p className="text-gray-500">Da:</p>
                      <p className="font-medium">{booking.health_record?.skin_condition || 'Bình thường'}</p>
                      
                      <p className="text-gray-500">Lông:</p>
                      <p className="font-medium">{booking.health_record?.coat_condition || 'Bình thường'}</p>
                      
                      <p className="text-gray-500">Tai:</p>
                      <p className="font-medium">{booking.health_record?.ear_condition || 'Bình thường'}</p>

                      <p className="text-gray-500">Mắt:</p>
                      <p className="font-medium">{booking.health_record?.eye_condition || 'Bình thường'}</p>

                      <p className="text-gray-500">Móng:</p>
                      <p className="font-medium">{booking.health_record?.nail_condition || 'Đã cắt gọn'}</p>
                    </div>
                    {booking.health_record?.behavior_observed && (
                      <div className="pt-2 border-t border-gray-50 mt-2">
                        <p className="text-gray-500 text-xs mb-1">Hành vi quan sát được:</p>
                        <p className="font-medium">{booking.health_record.behavior_observed}</p>
                      </div>
                    )}
                    {booking.health_record?.wound_description && (
                      <div className="pt-2 border-t border-gray-50 mt-2">
                        <p className="text-red-500 text-xs mb-1 flex items-center gap-1"><AlertTriangle size={12}/> Vết thương / Dị ứng:</p>
                        <p className="font-medium text-red-700">{booking.health_record.wound_description}</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            {/* History Diaries */}
            <div className={`px-6 pb-6 flex-1 overflow-y-auto custom-scrollbar ${viewMode === 'history' ? 'pt-6' : ''}`}>
              <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2 sticky top-0 bg-gray-50/90 backdrop-blur py-2 z-10">
              ...
              {(!booking.diaries || booking.diaries.length === 0) ? (
                ...
                <div className={`grid gap-4 ${viewMode === 'history' ? 'grid-cols-2' : 'grid-cols-1'}`}>
                  ...
                  <div className="flex flex-wrap gap-2.5">
                    {statusOptions.map(st => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm transition-all border shadow-sm ${
                          formData.status === st 
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-between items-center shrink-0">
          ...
          <div className="flex gap-4">
            ...
            >
              <Save size={18} /> Lưu nhật ký (trước 20:00)
            </button>
          </div>
          ) : (
            <button 
