...
export default function OrdersList() {
  ...
  const onEditShipping = () => {
    handleAction(() => {
      alert('Chưa hỗ trợ API sửa địa chỉ Supabase trong demo này');
    });
  };
          ...
          {TABS.map(tab => {
            ...
            return (
              <button
                ...
                className={`pb-3 px-2 font-bold text-sm whitespace-nowrap border-b-2 transition-all flex items-center gap-1.5 ${activeTab === tab.id ? 'border-chloro text-chloro' : 'border-transparent text-gray-400 hover:text-gray-700'
                  }`}
              >
                {tab.label}
                <span className={`px-1.5 py-0.5 rounded-full text-[10px] ${activeTab === tab.id ? 'bg-chloro text-white' : 'bg-gray-100 text-gray-500'
                  }`}>
                  {count}
                </span>
              </button>
